...
Public Class FFHumanAggregateMatrix
...
#End Region
    ...
    Private Sub LoadActualData()
        ...
        For Each activityParameter As AdminFfActivityParametersGetModel In listGet
            For Each txt As WebControls.TextBox In cls.FindCtrl(allCtrl, pnlHumanAggregateMatrix, GetType(WebControls.TextBox))
            ...
            For Each ddl As WebControls.DropDownList In cls.FindCtrl(allCtrl, pnlHumanAggregateMatrix, GetType(WebControls.DropDownList))
                'ParseParameter
            ...
            For Each txtDT As EIDSSControlLibrary.CalendarInput In cls.FindCtrl(allCtrl, pnlHumanAggregateMatrix, GetType(EIDSSControlLibrary.CalendarInput))
                'ParseParameter
            ...
            For Each chk As WebControls.CheckBox In cls.FindCtrl(allCtrl, pnlHumanAggregateMatrix, GetType(WebControls.CheckBox))
                'ParseParameter
...
#Region "FlexForm - RiskFactors"
    ...
    Private Sub LoadFlexFormTemplate(formTemplate As Long, Optional cleanUp As Boolean = False)
        ...
        Dim tbl As New Table With {
                .ID = "tblMatrixHumanAggregate",
                .Width = New Unit("100%"),
                .CellSpacing = 3,
                .CellPadding = 2
            }

        'Predefined Stub values.. 
        '        idfVersion      idfRow	        idfsParameter	strDefaultParameterName	idfsParameterValue	NumRow	strNameValue	            idfsSection	    langid
        '        58093790000000  58093800000000	226890000000	Diagnosis	            57972240000000	    0	    Other acute viral hepatitis	52630110000000	en
        '        58093790000000  58093800000000	229630000000	ICD-10 Code	            B17.0; B17.8	    0		                            52630110000000	en
        ...
        For Each ParameterTemplateRecord As AdminFfParameterTemplateGetModel In list
            ...
            'Select All the records that have the NumRow as 0
            'If ParameterTemplateRecord.NumRow = 0 Then
            ...
            '        Dim HeaderRowFound As TableHeaderRow = CType(pnlHumanAggregateMatrix.FindControl("row" + Item.idfVersion.ToString() + "_" + Item.idfRow.ToString()), TableHeaderRow)

            '        If Not HeaderRowFound Is Nothing Then

            '            Dim row As New TableHeaderRow
            '            'row.ID will be prefixed with ROW + idfVersion + idfRow ID
            '            'row.ID = "row" + Item.idfVersion.ToString() + "_" + Item.idfRow.ToString()
            ...
            '            ' Child label ID will the LBL + idfRow ID +  idfParameter ID 
            '            'childLabel.ID = "lbl" +item.idfRow.ToString() + "_" + item.idfParameter.ToString()
            '            'childlabel.Text = item.strDefaultParameterName.ToString()
            '            cell1.Controls.Add(childLabel)
            '            cell1.HorizontalAlign = HorizontalAlign.Left
            '            row.Cells.Add(cell1)

            '            CreateHeaderRow(ParameterTemplateRecord)
            '            CreateHeaderColumn(ParameterTemplateRecord)
            '        Else
            '            ' HeaderRowFound exists - so just append a new Column to this table. 

            '        End If

            '    Else
            '            tbl.Rows.Add(CreateHeaderRow(ParameterTemplateRecord))
            '    End If
            'End If
        Next
        ...
        For Each ParameterTemplateRecord As AdminFfParameterTemplateGetModel In list
            ...
            If Not ParameterTemplateRecord.idfsSection Is Nothing Then
                ...
                Dim tblSectionRetrieved As Table = CType(pnlHumanAggregateMatrix.FindControl("tblSection" + ParameterTemplateRecord.idfsSection.ToString()), Table)
        ...
        pnlHumanAggregateMatrix.Controls.Add(tbl)
    ...
    Private Function CreateHeaderRow(item As AdminFfParameterTemplateGetModel, Optional cleanUp As Boolean = False) As TableRow
    ...
    Private Function CreateHeaderColumn(item As AdminFfParameterTemplateGetModel) As TableCell
        If FlexibleFormAppService Is Nothing Then
            FlexibleFormAppService = New FlexibleFormServiceClient()
        End If
    ...
    Private Function CreateColumn(item As AdminFfParameterTemplateGetModel, Optional cleanUp As Boolean = False) As TableCell

        'Private Function CreateColumn(item As AdminFfParameterTemplateGetModel, Optional cleanUp As Boolean = False) As TableRow
    ...
    Private Function ValidateSection(sectionID As Long) As Boolean
        Dim convertedTable As Table = CType(pnlHumanAggregateMatrix.FindControl("tblSection" + sectionID.ToString()), Table)
    ...
    Private Sub InsertSection(sectionID As Long, sectionName As String, Optional verifTableID As String = Nothing)
        ...
        If Not verifTableID Is Nothing Then
            Dim verifTable As Table = CType(pnlHumanAggregateMatrix.FindControl(verifTableID), Table)
            ...
            End If
        Else
            pnlHumanAggregateMatrix.Controls.Add(pnlSection)
        End If
