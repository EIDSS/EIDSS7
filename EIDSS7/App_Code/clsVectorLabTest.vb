    ...
    Public Class clsVectorLabTest
        ...
        Public Function ListAll(Optional args() As String = Nothing) As DataSet Implements IEidssEntity.ListAll
            ...
            Try
                ...
                KeyValPair.Add(New clsCommon.Param() With {.ParamName = "@idfVector", .ParamValue = args(0).ToString(), .ParamMode = "IN"})
                KeyValPair.Add(New clsCommon.Param() With {.ParamName = "@idfVectorSurveillanceSession", .ParamValue = args(1).ToString(), .ParamMode = "IN"})
                KeyValPair.Add(New clsCommon.Param() With {.ParamName = "@LangId", .ParamValue = GetCurrentLanguage(), .ParamMode = "IN"})
                Dim oTuple = oService.GetData(GetCurrentCountry(), "VectorLabTestsGetList", oComm.KeyValPairToString(KeyValPair))
                oDS = oTuple.m_Item1
                oDS.CheckDataSet()
