<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="VeterinarySanitaryActionMatrix.aspx.vb"    MasterPageFile="~/NormalView.Master" Inherits="EIDSS.VeterinarySanitaryActionMatrix" meta:resourcekey="PageResource1" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="EIDSSBodyCPH" runat="server">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <div class="row">
                ...
                <div class="col-md-6">
                    <asp:Label ID="Label3" runat="server" CssClass="form-control" Text="Select Versions"></asp:Label>
                    <br />
    ...
    <div class="panel panel-default">
        <div class="panel-heading ">
            ...
            <h2>
                <asp:Label ID="Label5" runat="server" Text="Sanitary Actions Matrix" meta:resourcekey="Label5Resource1"></asp:Label></h2>
        ...
        <div class="panel-body">
            ...
            <asp:Table ID="HumanAggregateCaseTable" ClientIDMode="Static" runat="server" CssClass="table dataTable" meta:resourcekey="HumanAggregateCaseTableResource1">
                <asp:TableHeaderRow TableSection="TableHeader" VerticalAlign="Top" meta:resourcekey="TableHeaderRowResource1">
                    ...
                    <asp:TableHeaderCell Width="20%" meta:resourcekey="TableHeaderCellResource2">Sanitary Action</asp:TableHeaderCell>

                    <asp:TableHeaderCell Width="20%" meta:resourcekey="TableHeaderCellResource3">Sanitary Action Code</asp:TableHeaderCell>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
                        <ContentTemplate>
                            <uc1:MeasureReferenceEditorUserControl runat="server" ID="MeasureReferenceEditorUserControl1"  />
                        </ContentTemplate>
                    </asp:UpdatePanel>
                </div>
    ...
    <script type="text/javascript">
        ...
        * ********************************************************************************/
        ...
        $(document).ready(function () {
            ...
            //PageMethods.GetAllDiseases(LoadedSanitaryActions);
            PageMethods.GetAllSanitaryActions(LoadedSanitaryActions)
        ...
        function LoadedSanitaryActions(data) {
            diseaseData = data;
            sanitaryForGridDropDown = JSON.parse(diseaseData);
        }
       ...
       //Callback to get Matrix Version and load into dropDownList
        ...
        function SaveMatrix() {
            ...
            table.rows().eq(0).each(function (index) {
                ...
                var selectedVersion = $("#VersionDropDown").gorillaDropdown("selected");
                if (row.data()[3] != undefined & row.data()[3] != "") {
                    NewMatrixItem.intNumRow = row.data()[0];
                    NewMatrixItem.idfVersion = selectedVersion.value;
                    NewMatrixItem.idfsSanitaryAction = row.data()[3];
                    NewMatrixItem.idfAggrSanitaryActionMTX = row.data()[4];
                    MatrixObjectList.push(NewMatrixItem);
                    Duplicates.push(row.data()[3]);
                }
            });
        ...
        function GetMatrixByVersion(versionId) {
            //alert(versionId);
            var endpoint = baseRoute + '/api/Configuration/ConfAdminSanitaryMatrixReportGetGetAsync?Version';
            // console.log(baseRoute);
            $.ajax({
                url: endpoint,
                type: 'GET',
        ...
        function BuildTableWithData(data) {
            ...
            for (var i = 0; i < deserializedHumanDiseaseObj.length; i++) {
                ...
                item.push(deserializedHumanDiseaseObj[i].intNumRow.toString());
                 item.push(deserializedHumanDiseaseObj[i].idfsSanitaryAction);

                if (deserializedHumanDiseaseObj[i].strActionCode == undefined | deserializedHumanDiseaseObj[i].strActionCode == "") {
                    item.push("");
                }
                else {
                    item.push(deserializedHumanDiseaseObj[i].strActionCode); 
                }

                item.push(deserializedHumanDiseaseObj[i].idfsSanitaryAction);
                item.push(deserializedHumanDiseaseObj[i].idfAggrSanitaryActionMTX.toString());

                tableRow.intNumRow = deserializedHumanDiseaseObj[i].intNumRow;
                tableRow.idfsSanitaryAction = deserializedHumanDiseaseObj[i].idfsSanitaryAction;
                tableRow.strDefault = deserializedHumanDiseaseObj[i].strDefault;
                tableRow.idfHumanCaseMtx = deserializedHumanDiseaseObj[i].idfAggrSanitaryActionMTX;
        ...
        function DeleteMatrixRecord() {
            ...
            if (row.data()[4] != undefined & row.data()[4] != "") {
                NewMatrixItem.intNumRow = row.data()[0];
                NewMatrixItem.idfVersion = selectedVersion.value;
                NewMatrixItem.idfsDiagnosis = row.data()[3];
                NewMatrixItem.idfAggrHumanCaseMtx = row.data()[4];
            }
        ...
        function onDeleteMatrixSucess(data) {
            if (data !== "SUCCESS") {
                $("#generalMessageModalText").html(data);
            } else {
                $("#generalMessageModalText").html($("#RecordDeletedSuccessfullMessage").val());
            }
        ...
        function createSelect(data, type, row, meta) {
            ...
            for (var i = 0; i < sanitaryForGridDropDown.length; i++) {
                if (data == sanitaryForGridDropDown[i].idfsBaseReference) {
                     sel += "<option   value = '" + sanitaryForGridDropDown[i].idfsBaseReference + "' selected >" + sanitaryForGridDropDown[i].strDefault + "</option>";
                }
                else {
                    sel += "<option   value = '" + sanitaryForGridDropDown[i].idfsBaseReference + "' >" + sanitaryForGridDropDown[i].strDefault + "</option>";
                }
        ...
        $(document).on('change', '.tableDropDown', function () {
            ...
            var table = $('#HumanAggregateCaseTable').DataTable();
            var cell3 = table.cell(rowId, 3);
            var cell2 = table.cell(rowId, 2);
            cell3.data(selectedValue);
            var selectedSanitaryIndex = item.get(0).selectedIndex - 1;
            var strActionCode;
            if (sanitaryForGridDropDown[selectedSanitaryIndex] == undefined) {
                strActionCode = "";
                cell2.data("");
            }
            else {
                if (sanitaryForGridDropDown[selectedSanitaryIndex].strActionCode == "") {
                    strActionCode = "";
                }
                else {
                    strActionCode = sanitaryForGridDropDown[selectedSanitaryIndex].strActionCode;
                }
                cell2.data(strActionCode);
            }
        });
