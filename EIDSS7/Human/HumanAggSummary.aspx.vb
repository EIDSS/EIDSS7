...
Public Class HumanAggSummary
    ...
    Dim AggsToSummarizeList As List(Of String) = New List(Of String)
    Dim selectCount As Int64 = 0
...
#Region "Common"
    ...
    Private Sub AddToSelectedGrid(CaseId As String)


        Try
            ' This just adds them to a list


            gvSelectedHAC.DataSource = Nothing


            'How do I add an item to the gridview 1 at a time or a list at a time
    ...
    End Sub
    Private Sub FillSelectedGrid(Optional sGetDataFor As String = "GRIDROWS",
                                        Optional bRefresh As Boolean = False)


        Try
            ' This just adds them to a list
            Dim AggsToSummarizeList1 As List(Of String) = New List(Of String)

            Dim CheckBx As New List(Of Control)

            CheckBx.Clear()
            '        After CheckBox has been selected then use this code to get the list
            ...
            For Each chk As WebControls.CheckBox In FindControlList(CheckBx, gvHAC, GetType(WebControls.CheckBox))

                ' Get the strcasevalue(s)
                If chk.Checked Then

                    StrCaseId = hdnCaseReportUID.Value


                    'AddToSelectedGrid(StrCaseId)
                    AggsToSummarizeList1.Add(StrCaseId)
                End If
            ...
            FillAggSelectedList(pageIndex:=1, paginationSetNumber:=1)

            gvSelectedHAC.PageIndex = 0
    ...
    Private Sub FillAggSelectedList(pageIndex As Integer, paginationSetNumber As Integer)
...
#Region "Search Criteria"
    ...
    Private Sub ClearSearch()
        ...
        btnShowSearchCriteria.Visible = False
        searchResults.Visible = False
        btnSelect.Attributes.CssStyle.Add(HtmlTextWriterStyle.Display, "none")
        btnSelectAll.Attributes.CssStyle.Add(HtmlTextWriterStyle.Display, "none")
        btnCalculateSummary.Attributes.CssStyle.Add(HtmlTextWriterStyle.Display, "none")
    End Sub
    ...
    Private Sub showClearandSearchButtons(ByVal show As Boolean)
        ...
        btnSelect.Attributes.CssStyle.Add(HtmlTextWriterStyle.Display, "inline")
        btnSelectAll.Attributes.CssStyle.Add(HtmlTextWriterStyle.Display, "inline")
        btnCalculateSummary.Attributes.CssStyle.Add(HtmlTextWriterStyle.Display, "inline")
        If show Then
...
#Region "Sorting"
    ...
    End Sub
    Private Sub FillSummaryList()
        Try


            'checked box from the list  - Get them from AggsToSummarizeList
            Dim StrCaseIDAggSum As String
            For Each StrCaseIDAggSum In AggsToSummarizeList                ' Pass these values to the functions that summarizes the Flexform data based on CaseID
                'CalculateSummary get the data based on the 
                'get the aggregate data from the needed table and addit up to be displayed on the screen and stored in a table
            Next
    ...
    Private Sub DisplayBtnsForAddAgg()
        ...
        ' btnSelect.Visible = True  Do not need
        btnSelectAll.Visible = True
        btnCalculateSummary.Visible = True
...
#Region "Events"
    ...
    Protected Sub btnSearch_Click(sender As Object, e As EventArgs)
        ...
        If ValidateForSearch() Then
            ...
            btnSelectAll.Visible = True
            'btnSelect.Visible = True  not needed
            btnCalculateSummary.Visible = True
        ...
        End If
    End Sub
    Protected Sub chkCaseSummary_Click(sender As Object, e As EventArgs)

        Try
            If (sender.checked) Then
                hdnCaseReportUID.Value = sender.Attributes("value")
                'Add each check item to list
                AggsToSummarizeList.Add(hdnCaseReportUID.Value)
                hdnCaseReportSummaryCount.Value = AggsToSummarizeList.Count.ToString()
    ...
    End Sub
    Protected Sub chkCaseSelectedSummary_Click(sender As Object, e As EventArgs)
    ...
    Protected Sub btnSelect_Click(sender As Object, e As EventArgs)
        ...
        'Add all selected Aggregates to gvSelected
        btnCancelAgg.Visible = False
        btnCancelSearch.Visible = True
    ...
    Protected Sub btnSelectAll_Click(sender As Object, e As EventArgs)
        ...
        'Add all  Aggregates to gvSelected
        btnCancelAgg.Visible = False
        btnCancelSearch.Visible = True
    ...
    End Sub
    Protected Sub btnShowSummary_Click(sender As Object, e As EventArgs)

        ' This button would show a popup of the shosen list of aggregataed to summarize - Not Needed
        'Extra step in code for no reason -- INstead just calculate selected otems and display summary
        'See btnCalculateSummary
        Dim CheckBx As New List(Of Control)
        ...
        For Each chk As WebControls.CheckBox In FindControlList(CheckBx, gvHAC, GetType(WebControls.CheckBox))
            ...
            ' Get the strcasevalue(s)
            Dim StrCaseId As String = hdnCaseReportUID.Value
            '<%#Eval("strCaseID")%>
    ...
    Protected Sub btnCalculateSummary_Click(sender As Object, e As EventArgs)
        ...
        For Each StrCaseIDAggSum In AggsToSummarizeList                ' Pass these values to the functions that summarizes the Flexform data based on CaseID
            ...
            humanAggGetParameters.strSearchCaseId = StrCaseIDAggSum
            ...
            'THis return data for the strcase list that is provided
            'Here is where we add up the summary and return it to the screen

        Next



        'Display Aggregate FlexForm
        If Not LoadTemplates() Is Nothing Then
    ...
    Protected Sub btnNext_Click(sender As Object, e As EventArgs)
        ...
        btnCalculateSummary.Visible = False
    ...
    Protected Sub btnReturnToAggrRecord_Click(sender As Object, e As EventArgs)
        ...
        search.Visible = True
        searchResults.Visible = True
        btnCancelSearch.Visible = True ' Since Search results are being displayed
