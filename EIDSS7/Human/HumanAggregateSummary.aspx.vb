...
Imports EIDSS.EIDSS
    ...
    Public Class HumanAggregateSummary
...
#Region "Initialize"
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            'HumanAggregateFlexForm.FormType = 10034010
            'HumanAggregateFlexForm.DiagnosisID = 70023994201 'Test Value
            'HumanAggregateFlexForm.ObservationID = Long.Parse(1)
...
#Region "Events"
        ...
        Protected Sub btnNewHAC_Click(sender As Object, e As EventArgs)
            ...
            'HumanAggregateFlexForm.DiagnosisID = 70023994201 'Test Value
            'HumanAggregateFlexForm.ObservationID = Long.Parse(1)
            'HumanAggregateFlexForm.FormType = 10034012
...
#Region "FlexForm - Matrix"
                ...
                For Each ParameterTemplateRecord As AdminFfParameterTemplateGetModel In list
                    ...
                    If Not ParameterTemplateRecord.idfsSection Is Nothing Then
                        ...
                        If Not tblSectionRetrieved Is Nothing Then
                            ...
                            CreateParameterColumn(ParameterTemplateRecord, parameterControlRow, loadMatxrixTable)
                        Else
        ...
        Protected Sub AddStaticColumnControl(parameterCount As Int16, parameterControlRow As TableRow, loadMatxrixTable As Table)
            'If HumanClient Is Nothing Then
            ...
            For Each diseasesRecord As GblDiseaseMtxGetModel In diseases
                If Count = 0 Then
                    Dim cell1 As New TableCell With
                {
                    ...
                    Dim headerLabel As New Label With
                {
                  .Text = diseasesRecord.strDefault,
                  .Width = New Unit("100%")
                }

                    cell1.Controls.Add(headerLabel)
                    parameterControlRow.Cells.AddAt(0, cell1)
                    ...
                    Dim cell2 As New TableCell With
                {
                    ...
                    Dim headerLabel2 As New Label With
                {
                    .Text = diseasesRecord.strIDC10,
                    .Width = New Unit("100%")
                }

                    cell2.Controls.Add(headerLabel2)
                    parameterControlRow.Cells.AddAt(1, cell2)
                    Count = 1
                Else
                        ...
                        Dim cell3 As New TableCell With
                {
                    ...
                    {
                        .ID = "txtString" + diseasesRecord.idfsBaseReference.ToString() + "_" + index.ToString(),
                        .Text = "",
                        .CssClass = "form-control"
                    }
        ...
        '    ScriptManager.RegisterClientScriptBlock(Me, Me.GetType, "alertMessage", "alert('test')", True)
        'End Sub
        ...
        Protected Sub LoadidfVersion()
            Try
                ...
                If Not formTypeCollection.FirstOrDefault().idfsMatrixType Is Nothing Then
                    Dim Matrixlist As List(Of AdminFfAggregateMatrixVersionGetListModel) = FlexibleFormService.GET_FF_AGGREGATEMATRIX(GetCurrentLanguage(), formTypeCollection.FirstOrDefault().idfsMatrixType).Result
        ...
        Private Function GenrateObservationId()
            Dim Generator As System.Random = New System.Random()
            Dim Min As Integer = 0
            Dim Max As Integer = 9999999


            Dim ObservationId = Generator.Next(Min, Max)
        ...
        Private Sub LoadActualData()
            ...
            Dim listGet As List(Of AdminFfActivityParametersGetModel) = FlexibleFormService.GET_FF_ActivityParameters(Convert.ToString(hdfidfCaseObservation), GetCurrentLanguage()).Result
