...
--##SUMMARY Select data for Country Report “Veterinary- sanitary measures” V4.
...
exec spRepVetProphilacticMeasures_Country 
'en', 
'2011-02-01', 
'2011-03-01', 
...
create  PROCEDURE [dbo].[spRepVetProphilacticMeasures_Country]
	(
...
-- "https://repos.btrp.net/BTRP/Project_Documents/08x-Implementation/Customizations/KZ/Reports/Specification_for_reports_development_Country_prophilactic-measure-KZ-V4.docx"
...
-- heder part of the table: all existing variants
 (
    SELECT DISTINCT idfsRegion, idfsSanitaryAction FROM @VetSanitaryActionMatrixValuesTable
    UNION
    SELECT DISTINCT idfsRegion,  idfsSanitaryAction FROM @VetSanitaryActionMatrixValuesTable_Year
 ) All_rows
-- translation 
...
-- add rows for all region that has NO data -----------------------------------------------------
INSERT INTO @ReportTable
(	idfsBaseReference
	,strRegionName
	,strMeasureType
