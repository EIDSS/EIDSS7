...
--##REMARKS Create date: 07.08.2013
...
exec  dbo.spDiagnosesAndGroups_SelectLookup 'en', 32, 10020001 -- 'dutStandartCase'
*/


CREATE procedure dbo.spDiagnosesAndGroups_SelectLookup (
	...
	@idfsGroupReference bigint = Null
)
as 
Begin
	if (@idfsGroupReference is null) set @idfsGroupReference = 19000156
...
OUTER APPLY (SELECT TOP 1 *
 ...
 WHERE gr.idfsDiagnosis = D.idfsDiagnosis and gr.intRowStatus = 0) AS g
...
select	  dn.idfsReference as idfsDiagnosisOrDiagnosisGroup
		...
		, CASE WHEN @HACode = 0 or @HACode is null or dn.intHACode is null or dn.intHACode & @HACode > 0
			THEN  
				CASE WHEN exists(select * from @ResultTable where idfsDiagnosisGroup = dn.idfsReference and intRowStatus = 0)
					THEN  dn.intRowStatus
					ELSE 1
				END
			ELSE 1
		...
		,YNU.idfsReference as [intIsDeleted]
		,YNU.name as [strIsDeleted]
from	dbo.fnReferenceRepair(@LangID, @idfsGroupReference) dn --rftDiagnosisGroup
Inner join dbo.fnReference(@LangID, 19000100) YNU On (case when dn.intRowStatus = 0 then 10100002 else 10100001 end) = YNU.idfsReference
--where 
--	exists(select * from @ResultTable where idfsDiagnosisGroup = dn.idfsReference and intRowStatus = 0)
