...
EXECUTE spLaboratorySection_GetByFieldBarcode
   '12345'
  ,'en'
...
create PROCEDURE dbo.spLaboratorySection_GetByFieldBarcode
...
--select * from dbo.fn_LaboratorySectionGetByFieldBarcode_SelectList(@LangID) as tlbMaterial
--where 
		--isnull(tlbMaterial.strFieldBarcode, '') = case 
		--	when nullif(@idfMaterial,0) = 0 then @strFieldBarcode
		--	else isnull(tlbMaterial.strFieldBarcode,'')
		--end
		--and isnull(tlbMaterial.idfSendToOffice,0) = isnull(nullif(@idfSendToOffice,0), isnull(tlbMaterial.idfSendToOffice,0))
		--and isnull(tlbMaterial.idfMaterial,0) = isnull(nullif(@idfMaterial,0), isnull(tlbMaterial.idfMaterial,0))
...
from (
	...
	from tlbMaterial
		...
		on			OfficeSendTo.idfOffice = tlbMaterial.idfSendToOffice
			...
			where
		tlbMaterial.idfMaterial = @idfMaterial
