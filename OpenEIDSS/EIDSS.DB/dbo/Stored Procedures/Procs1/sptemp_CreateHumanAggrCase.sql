...
--##SUMMARY Procedure produces a set of human aggr cases with the specified parameters
...
CREATE PROC [dbo].[sptemp_CreateHumanAggrCase](
...
BEGIN
	...
	BEGIN	
		...
		SET @SentByPerson = ISNULL(@SentByPerson, @person)
		...
		EXEC dbo.spFFGetActualTemplate 
			...
			,@idfsFormType=10034012 /*Human Aggregate Case*/
			,@blnReturnTable = 0
		...
		DECLARE @Version BIGINT = (
									...
									WHERE tamvh.idfsMatrixType = 71190000000/*Human Aggregate Case*/
		...
		EXECUTE spAggregateCaseHeader_Post
			...
			,@idfsAggrCaseType = 10102001 /*AggregateCase*/
			,@idfsAdministrativeUnit = @AdministrativeUnitLocal
			...
			,@datStartDate = @PeriodStartDateLocal
			,@datFinishDate = @PeriodFinishDateLocal
			,@strCaseID = NULL
	END
	ELSE 
