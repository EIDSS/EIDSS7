...
from		dbo.tlbVetCase					as tVetCase
	...
	inner join	fnAnimalList(@LangID)		as fnAnimalList
	 ...
	 left join	dbo.fnReferenceRepair(@LangID, 19000012 ) as rfCaseType  --rftCaseType
			on	tVetCase.idfsCaseType = rfCaseType.idfsReference
	-- Get Farm
	 left join	(		
	 ...
	 left join	dbo.fnReferenceRepair(@LangID, 19000019 ) tDiagnosis --rftDiagnosis
	  ...
	  group by  tDiagnosis.name , rfFarmRegion.[name], SpecimenType.name, rfCaseType.[name]
