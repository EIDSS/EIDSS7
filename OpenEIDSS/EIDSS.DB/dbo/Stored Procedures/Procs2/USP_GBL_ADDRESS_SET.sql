--*************************************************************
-- Name 				: USP_GBL_ADDRESS_SET
-- Description			: SET Address
--          
-- Author               : Mandar Kulkarni
-- Revision History
--		Name       Date       Change Detail
-- Stephen Long    04/21/2019 Fixed the check for location 
--                            shared versus location on the 
--                            update portion.
--
--
-- Testing code:
--*************************************************************
CREATE PROCEDURE [dbo].[USP_GBL_ADDRESS_SET]
(
...
--		IF  @idfGeoLocation IS NULL RETURN-1;
		...
		IF @blnForeignAddress <> 0 AND @strPostCode IS NOT NULL
			...
			END
		IF (EXISTS(SELECT * FROM dbo.tlbGeoLocation WHERE idfGeoLocation = @idfGeoLocation) AND (ISNULL(@blnGeoLocationShared, 0) <> 1))
			BEGIN
