...
BEGIN
	...
	IF  (NOT EXISTS(SELECT idfAggrDiagnosticActionMTX FROM tlbAggrDiagnosticActionMTX WHERE idfsSpeciesType = @idfsSpeciesType AND intRowStatus = 0) AND
		...
		UPDATE trtStringNameTranslation SET intRowStatus = 1
			WHERE idfsBaseReference = @idfsSpeciesType
		END
		ELSE IF  (EXISTS(SELECT idfAggrDiagnosticActionMTX FROM tlbAggrDiagnosticActionMTX WHERE idfsSpeciesType = @idfsSpeciesType AND intRowStatus = 0) OR
		...
		EXISTS(SELECT CampaignToSampleTypeUID FROM CampaignToSampleType WHERE idfsSpeciesType = @idfsSpeciesType AND intRowStatus = 0)  OR
